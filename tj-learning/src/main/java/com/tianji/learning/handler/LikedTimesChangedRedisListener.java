...
@Slf4j
public class LikedTimesChangedRedisListener {
	...
	public void likeTimesChanged(List<LikedTimesDTO> dto){
		if(dto == null) throw new BizIllegalException("MQ信息参数异常！");
		log.debug("点赞数同步");
		List<InteractionReply> replies = dto.stream().map(d -> {
			InteractionReply reply = new InteractionReply();
			reply.setId(d.getBizId());
			reply.setLikedTimes(d.getLikedTimes());
			return reply;
