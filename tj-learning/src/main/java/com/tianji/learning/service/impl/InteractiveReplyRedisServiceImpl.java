...
public class InteractiveReplyRedisServiceImpl extends ServiceImpl<InteractionReplyMapper, InteractionReply> implements IInteractionReplyService {
	...
	public PageDTO<ReplyVO> pageQueryReply(ReplyPageQuery queryDto) {
		...
		List<LikedTimesDTO> repliesLikeTimes = remarkClient.bizLikedTimesBatch("QA", ids);
		Map<Long, Integer> repliesLikeTimesMap = repliesLikeTimes.stream().collect(Collectors.toMap(LikedTimesDTO::getBizId, LikedTimesDTO::getLikedTimes));
		//查询用户信息
		...
		List<ReplyVO> list = records.stream().map(record -> {
			...
			vo.setLikedTimes(repliesLikeTimesMap.get(record.getId()));
